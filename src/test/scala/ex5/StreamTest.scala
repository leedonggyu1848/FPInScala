...
class StreamTest extends AnyFlatSpecLike with Matchers {
  ...
  it should "take elements while the predicate is true" in {
    ...
    Stream(1, 2, 3, 4, 5).takeWhile(_ > 100).toList shouldBe Nil
  ...
  it should "take elements while the predicate is true" in {
    Stream(1, 2, 3, 4, 5).takeWhileViaFoldRight(_ < 3).toList shouldBe List(1, 2)
    Stream(1, 2, 3, 4, 5).takeWhileViaFoldRight(_ > 100).toList shouldBe Nil
    Stream().takeWhileViaFoldRight(_ => true).toList shouldBe Nil
  }

  behavior of "map"
  it should "map elements" in {
    Stream(1, 2, 3).map(_ + 1).toList shouldBe List(2, 3, 4)
    Stream().map(_ => 1).toList shouldBe Nil
  }

  behavior of "filter"
  it should "filter elements" in {
    Stream(1, 2, 3, 4, 5).filter(_ % 2 == 0).toList shouldBe List(2, 4)
    Stream(1, 2, 3, 4, 5).filter(_ => false).toList shouldBe Nil
  ...
  it should "append two streams" in {
    ...
    Stream().append(Stream(1, 2, 3)).toList shouldBe List(1, 2, 3)
    Stream(1, 2, 3).append(Stream()).toList shouldBe List(1, 2, 3)
  }

  behavior of "flatMap"
  it should "flat map elements" in {
    Stream(1, 2, 3, 4, 5).flatMap(e => Stream(e + 1)).toList shouldBe List(2, 3, 4, 5, 6)
    Stream[Int]().flatMap(e => Stream(e + 1)).toList shouldBe Nil
  }
  ...
  it should "return an infinite stream of a given value" in {
    ...
    Stream.constant(1).take(0).toList shouldBe Nil
  }
  ...
  behavior of "unfold"
  it should "return an infinite stream of integers starting from a given value" in {
    Stream.unfold(1)(e => Some((e, e + 1))).take(5).toList shouldBe LazyList.unfold(1)(e => Some((e, e + 1))).take(5).toList
  }
  ...
  it should "map elements" in {
    ...
    Stream().mapViaUnfold(_ => 1).toList shouldBe Nil
  }

  behavior of "takeViaUnfold"
  it should "take the first n elements of a stream" in {
    Stream(1, 2, 3).takeViaUnfold(2).toList shouldBe List(1, 2)
    Stream(1, 2, 3).takeViaUnfold(0).toList shouldBe Nil
    Stream().takeViaUnfold(10).toList shouldBe Nil
  }

  behavior of "takeWhileViaUnfold"
  it should "take elements while the predicate is true" in {
    Stream(1, 2, 3, 4, 5).takeWhileViaUnfold(_ < 3).toList shouldBe List(1, 2)
    Stream(1, 2, 3, 4, 5).takeWhileViaUnfold(_ > 100).toList shouldBe Nil
    Stream().takeWhileViaUnfold(_ => true).toList shouldBe Nil
  }

  behavior of "zipWith"
  it should "zip two streams with a given function" in {
    Stream(1, 2, 3, 4, 5).zipWith(Stream(1, 2, 3, 4, 5))(_ + _).toList shouldBe List(2, 4, 6, 8, 10)
    Stream(1, 2, 3).zipWith(Stream(1, 2, 3, 4, 5))(_ + _).toList shouldBe List(2, 4, 6)
    Stream(1, 2, 3, 4, 5).zipWith(Stream(1, 2, 3))(_ + _).toList shouldBe List(2, 4, 6)
    Stream(1, 2, 3).zipWith(Stream.empty[Int])(_ + _).toList shouldBe Nil
    Stream.empty[Int].zipWith(Stream(1, 2, 3))(_ + _).toList shouldBe Nil
  }
  ...
  it should "zip two streams with a given function" in {
    ...
    Stream(1, 2, 3).zipAll(Stream(1, 2, 3)).foldRight(List.empty: List[Int])(add) shouldBe List(2, 4, 6)
    Stream(1, 2).zipAll(Stream(1, 2, 3)).foldRight(List.empty: List[Int])(add) shouldBe List(2, 4, 3)
    Stream(1, 2, 3).zipAll(Stream(1, 2)).foldRight(List.empty: List[Int])(add) shouldBe List(2, 4, 3)
    Stream.empty[Int].zipAll(Stream.empty[Int]).foldRight(List.empty: List[Int])(add) shouldBe Nil
  }
  ...
  it should "return all tails of a stream" in {
    ...
    Stream.empty[Int].tails.map(_.toList).toList shouldBe List(Nil)
  }
